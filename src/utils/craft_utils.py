...
def hard_negative_mining(pred, target, cfg):
    ...
    positive = target >= cfg.craft.THRESHOLD_POSITIVE
    negative = target <= cfg.craft.THRESHOLD_NEGATIVE
    ...
    '''
    s = time.time()
    indices = min(max(1000, 3 * torch.sum(positive)), torch.sum(negative))
    #negative_loss_top = torch.sort(negative_loss, descending=True).values
    #negative_loss_top = torch.topk(negative_loss, indices)
    print('del', time.time() - s)
    '''
