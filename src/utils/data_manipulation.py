...
gaussian_heatmap = cv2.cuda_GpuMat()
gaussian_heatmap.upload(gaussian_heatmap_temp)

gaussian_heatmap_aff_temp = gaussian_heatmap_aff
gaussian_heatmap_aff = cv2.cuda_GpuMat()
gaussian_heatmap_aff.upload(gaussian_heatmap_aff_temp)
...
def four_point_transform(image, pts, size):
    ...
    warped = cv2.cuda.warpPerspective(image, cv2.getPerspectiveTransform(dst, pts), (width, height))
...
def add_character(image, bbox, heatmap=gaussian_heatmap):
    ...
    image = cv2.cuda.max(image, transformed)
    ...
    if len(image_size) == 2:
        ...
        for word, count in zip(text, word_count):
            ...
            for char_num in range(count-1):
                try:
                    src, bbox = add_affinity(src, character_bbox[total_letters].copy(), character_bbox[total_letters+1].copy(), \
