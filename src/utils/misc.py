...
def link_to_word_bbox(to_find, word_bbox):
	...
	for word_i, word in enumerate(word_bbox):
		compare_word_polygons.append(Polygon(word.reshape([word.shape[0], 2])).buffer(0))

	compare_word_polygons = STRtree(compare_word_polygons)
	...
	for cont_i, cont in enumerate(to_find):
		...
		for b in compare_word_polygons.query_items(a):
			ratio_arr.append((compare_word_polygons._geoms[b].intersection(a).area / a.area, b))
		ratio_arr = sorted(ratio_arr, key=lambda x:(-x[0], x[1]))
		# if np.argmax(ratio) != ratio_arr[0][1]:
		# 	print(np.argmax(ratio), ratio_arr[0][1])
		# word_sorted_character[np.argmax(ratio)].append(ordered_bbox)
		if len(ratio_arr) != 0:
			word_sorted_character[0].append(ordered_bbox) if ratio_arr[0] == 0.0 else word_sorted_character[ratio_arr[0][1]].append(ordered_bbox)
		else:
